 ...
 *
 * 抓数据走 `force = false`（1.2.1 起）：让 [WidgetRefreshScheduler] 的 2 分钟合并窗口
 * 生效。边界闹钟常常在几分钟内连着响两次（例如跨天与开课前），每次都强制抓一遍
 * 只是把同一个结果重复取回来。
 *
...
class WidgetAlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent?) {
        ...
        runCatching { WidgetRefreshScheduler.refreshData(context) }
