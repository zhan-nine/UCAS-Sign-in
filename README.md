...
1. 下载最新安装包 [qingxin-signin-1.2.2-release.apk](https://github.com/zhan-nine/UCAS-Sign-in/raw/refs/heads/main/releases/qingxin-signin-1.2.2-release.apk)，也可以从项目的 [Releases 发布页](https://github.com/zhan-nine/UCAS-Sign-in/releases) 或仓库 [`releases/`](./releases/) 目录获取。
...
- **版本更新提示**：进入主页时自动检查本仓库的 GitHub Releases（**24 小时内最多一次**，间隔由耗电档位决定），发现更高版本时在主页提示并提供「去 GitHub 更新」；可选择「稍后」或「不再提示」（仅忽略该版本）。设置页「版本与更新」卡片**常显当前版本、远端最新版本与上次检查时间**（即使已是最新也能看出检查确实跑过），并可开关自动检查、手动检查、恢复已忽略的版本
...
- **三种「不打扰」语义刻意分开**：
  ...
  - **自动检查更新**（设置 → 版本与更新）—— 总开关，关掉后不再自动检查，手动「检查更新」仍可用。
- 设置页可随时查看当前版本号、手动检查、以及恢复已忽略的版本。
- 设置页页头常显 `v<当前版本>`，「版本与更新」卡片里除当前版本外还常显
  **远端最新版本**与**上次检查时间**。这样「已是最新」时也有内容可核对，
  能一眼区分「确定没有更新」与「压根没查过 / 查询失败」——后者以前在界面上
  无法分辨，看起来就像这个功能没有实现。
